...
export default function ProjectSidebar({ filteredProjects }: ProjectSidebarProps) {
  ...
  return (
    <>
      ...
      <div 
        className={`absolute top-4 left-4 bottom-4 w-[280px] z-10 transition-all duration-500 ease-in-out origin-left ${
        ...
        <div className="h-full bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-white/60">
          <div className="p-4 border-b border-gray-200/60 bg-white/50 flex items-center gap-2">
            ...
            <Button 
              variant="ghost" 
              size="icon" 
              className="shrink-0 h-10 w-10 text-gray-500 rounded-lg hover:bg-gray-200/50 transition-colors" 
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth">
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className={`flex items-center gap-3 p-3 border-b border-gray-100/80 cursor-pointer transition-all hover:bg-gray-50/90 ${
